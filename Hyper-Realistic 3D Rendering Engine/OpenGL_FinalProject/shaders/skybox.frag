...
void main()
{
    ...
    vec3 finalColor = rayleigh * mie * extinction;

    // Clouds
    float density = smoothstep(1.0 - cirrus, 1.0, fbm(pos.xyz / pos.y * 2.0 + time * 0.05)) * 0.3;
    finalColor = mix(finalColor, extinction * 4.0, density * max(pos.y, 0.0));

    // Cumulus Clouds
    for (int i = 0; i < 3; i++)
    {
      float density = smoothstep(1.0 - cumulus, 1.0, fbm((0.7 + float(i) * 0.01) * pos.xyz / pos.y + time * 0.3));
      finalColor = mix(finalColor, extinction * density * 5.0, min(density, 1.0) * max(pos.y, 0.0));
    }

    // Dithering Noise
    finalColor += noise(pos * 1000) * 0.01;
    
    // Stars
    if (fsun.y < 0.1) {
        float starThreshold = 0.9998; 
        float starNoise = hash(dot(normalize(pos), vec3(13.0, 79.0, 46.0)));
        if (starNoise > starThreshold) {
            // brightness
            float starIntensity = pow((starNoise - starThreshold) / (1.0 - starThreshold), 2.0) * 60.0;
            // flicker effect
            starIntensity *= (0.7 + 0.3 * sin(time * 3.0 + starNoise * 100.0));
            // fade stars in/out based on sun position
            float nightFactor = smoothstep(0.1, -0.2, fsun.y);
            finalColor += vec3(starIntensity) * nightFactor;
        }
    }
    
    finalColor = pow(1.0 - exp(-1.3 * finalColor), vec3(1.3));
