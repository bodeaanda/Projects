...
namespace gps {
    ...
    void Shader::loadShader(std::string vertexShaderFileName, std::string tessControlShaderFileName, 
        ...
        //read, parse and compile the tessellation control shader
        std::string tc = readShaderFile(tessControlShaderFileName);
        const GLchar* tessControlShaderString = tc.c_str();
        GLuint tessControlShader;
        tessControlShader = glCreateShader(GL_TESS_CONTROL_SHADER);
        glShaderSource(tessControlShader, 1, &tessControlShaderString, NULL);
        glCompileShader(tessControlShader);
        shaderCompileLog(tessControlShader);

        //read, parse and compile the tessellation evaluation shader
        std::string te = readShaderFile(tessEvalShaderFileName);
        const GLchar* tessEvalShaderString = te.c_str();
        GLuint tessEvalShader;
        tessEvalShader = glCreateShader(GL_TESS_EVALUATION_SHADER);
        glShaderSource(tessEvalShader, 1, &tessEvalShaderString, NULL);
        glCompileShader(tessEvalShader);
        shaderCompileLog(tessEvalShader);
